...
public:
    ...
    SingleLinkedList(std::initializer_list<Type> values) {
        InitializeSimpleLinkedList(values.begin(), values.end());
    }

    SingleLinkedList(const SingleLinkedList& other) {
        InitializeSimpleLinkedList(other.begin(), other.end());
    }
    ...
    */
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        assert(size_ > 0u);
        Node* erased_node = pos.node_->next_node;
    ...
    void Clear() noexcept {
        while (!this->IsEmpty()) {
            //Ищем на данный момент первый не фиктивный узел
            Node* current_node = head_.next_node;
            //Обновляем первый элемент списка
            head_.next_node = current_node->next_node;
            //А теперь освобождаем текущий узел, который только что затерли
            delete current_node;
        }
        size_ = 0u;
    }
...
private:
    ...
    void InitializeSimpleLinkedList(It begin, It end) {
        std::vector<Type> v;
        for (auto it = begin; it != end; ++it) {
            v.push_back(*it);
        }

        SingleLinkedList<Type> copy_list;
        for (auto it = v.rbegin(); it != v.rend(); ++it) {
            copy_list.PushFront(*it);
        }
        copy_list.size_ = v.size();
